"""BaseAgentSession — 通用会话底座（从 ChatSession 抽离的纯会话管理层）

核心职责：
1. 包装 ClaudeSDKClient，管理会话生命周期
2. 将 SDK 的 Message/Block 转换为 ChatEvent 事件流
3. 维护对话统计（轮次 / Token / 工具调用数 / 成本）
4. 检测上下文变更，平滑重启底层 Client

与 ChatSession 的关系：
   ChatSession 继承本底座，覆盖 _resolve_system_prompt() 注入 ContextProvider
   构建的 claude_code preset system_prompt。L0Router 将直接组合本底座，传
   system_prompt=str / permission_mode="default" / can_use_tool=guard 回调。

⚠️ 脑手解耦硬约束的落点（Step 1 新增）：
   - permission_mode：默认 "acceptEdits"（ChatSession 旧行为），L0Router 传 "default"
   - can_use_tool：默认 None（不挂载），L0Router 传 guard 回调实现动态权限
   - system_prompt：默认 None（由子类 _resolve_system_prompt 钩子构建）

⚠️ AsyncIterable prompt（Step 1 改造）：
   send() / _hot_restart_client() 的 query 调用统一用 _to_prompt_stream() 包装，
   使 can_use_tool 在 ask 态可触发；对 acceptEdits 模式无副作用（Spike G 验证）。

⚠️ 过渡态说明：
   BaseAgentSession 是 Phase 2 的通用会话底座，Phase 3 中
...
class SessionStats:
    ...
    def __init__(self) -> None:
        ...
        # ── 成本累计 ──
        self.total_cost_usd: float = 0.0  # 累计成本（美元）
...
class BaseAgentSession:
    """通用会话底座 — 不绑定特定 system_prompt 构建策略

    使用方式：

        session = BaseAgentSession(project_dir="./my-app")
        await session.start()
    ...
    扩展钩子：
       _resolve_system_prompt() — 默认返回 self.system_prompt；
       子类可覆盖以注入 ContextProvider 构建的 system_prompt。
    """
    ...
    def _to_prompt_stream(prompt: str):
        ...
        格式来自 SDK 内部 string→message 包装 (client.py:297-304)。

        ⚠️ 不设 session_id：SDK query() 仅在 msg 缺 session_id 时才按其
        session_id 参数（默认 "default"）填充 (client.py:308-310)。若硬编码
        session_id=""，SDK 跳过填充，CLI 收到空 session_id → 每轮当新会话，
        多轮记忆丢失。
        """
        async def _stream():
            yield {
    ...
    def _build_options(self, for_rebuild: bool = False) -> ClaudeAgentOptions:
        ...
        """
        # 通过钩子组装 system_prompt（子类可覆盖以注入 ContextProvider）
    ...
    async def send(self, prompt: str) -> AsyncIterator[ChatEvent]:
        ...
        try:
            await self._client.query(self._to_prompt_stream(prompt))
