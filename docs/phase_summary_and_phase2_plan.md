...
L0 常驻 session 用 `permission_mode="default"` + `allowed_tools=[Read,Glob,Grep,Bash]`（**业务写工具不进**，强制走 ask 态），`can_use_tool` 准入回调按当前 lane 拒绝越界工具：
...
async def _lane_guard(self, tool_name: str, tool_input: dict, ctx) -> PermissionResult:
    ...
    签名对齐 SDK client.py:408 (tool_name, tool_input, ctx)。
    仅在工具处于 ask 态时触发——故业务写必须不在 allowed_tools 内(§13 Run A/B)。"""
    if self._current_lane != "fast" and tool_name in BUSINESS_WRITE_TOOLS:
        return PermissionResultDeny(message="当前模式:业务代码修改由 claude code 子进程执行,L0 不得直接编辑代码文件。")
    return PermissionResultAllow()
```

**状态残留兜底**：入口 `handle_user_input` 将 `_current_lane` 重置为 `None`；回调对 `None` 按最严（等同 full）处理。

> **⚠️ Spike 修正(§13)**：`can_use_tool` 与 `PreToolUse` 都只在 ask 态生效；`PreToolUse` 只能 deny-at-ask 不能 allow，故 lane 放行只能靠 `can_use_tool`。`can_use_tool` 要求 AsyncIterable 流式 prompt（§4.3/架构师.md §4.3），string prompt 会 `ValueError`。
...
| Fast Lane 自执行 | 常驻 `ClaudeSDKClient.query()`（流式 AsyncIterable prompt） | `ClaudeSDKClient` + `allowed_tools=[Read,Glob,Grep,Bash]`（业务写不进，靠 `can_use_tool` 按 lane 放行） | Step 3 |
| Full 模式派发 | `asyncio.create_subprocess_exec` | `claude -p ...` CLI 调用（路径经 `shutil.which` 解析） | Step 4 |
...
| 文件路径 | 变更类型 | 改造量 | 说明 |
|---------|---------|:-----:|------|
| `core/base_session.py` | **新增** | 中 | **公共会话底座** — 从 `ChatSession` 抽离纯会话管理能力：SDK client 生命周期、自动重试/stderr 监控、取消信号处理、`can_use_tool` 权限回调挂载、session_store 持久化与续接。不含路由/命令/渲染逻辑。L0Router 内部常驻执行会话 + 路由临时会话均基于此接口。**⚠️ Spike 修正(架构师.md §4.3/§13)**：(1) `send()` 改造为 AsyncIterable prompt 流式模式——`can_use_tool` 仅在流式路径注册，string prompt 会 `ValueError`；(2) `permission_mode` 与 `can_use_tool` 均为可配参数——ChatSession 注入 `acceptEdits`+无 `can_use_tool`，L0Router 注入 `default`+`can_use_tool`（业务写工具不进 `allowed_tools`，强制走 ask 态触发回调） |
...
| `chat/repl.py` | **中改（入口切换）** | 中 | 增加模式开关（单 Agent / L0 Router），默认走 L0 模式。斜杠命令 `/fast`、`/full` 经 commands.py 拦截设 `forced_lane` 透传 `handle_user_input`（`/fast` 跳过路由、`/full` 仍路由拆任务但覆盖 lane，见 §5.1.3）；`/clear` 清空常驻会话历史；`/undo` 仅 Fast Lane 生效 |
...
| `core/agents/base_agent.py` | **保留（无关联）** | 0 | Phase 1 遗留，`run_agent()` 纯函数服务于 `harness run` 单次 CLI 路径（经 `orchestrator.py`），产出 `on_custom_event` dict。与 `BaseAgentSession`（类，产出 `ChatEvent`）职责重叠但事件格式不同，Phase 2 不统一，收尾时评估是否合并 |
...
| `/clear` | 全模式生效（`_full_active` 时拒绝） | 经 `router.rebuild()` 重建常驻会话，重置 `_current_lane` 为 `None` |
| `/context` | 全模式生效 | 展示常驻会话上下文用量（`cli.session.context_provider`） |
| `/stats` | **改两栏** | ① 常驻会话（`stats`：turns/tools/tokens/cost）② Full 累计（`_full_cost`/`_full_tokens`） |
| `/project` | 全模式生效（`_full_active` 时拒绝切换） | 显示 `cli.session.project_dir`；切换走 `router.rebuild(project_dir=...)` |
| `/model` | 全模式生效（`_full_active` 时拒绝切换） | 显示 `cli.session.model`；切换走 `router.rebuild(model=...)` |
| `/fast` | **新增** | 强制走 Fast Lane：设 `forced_lane="fast"` 透传 `handle_user_input`，跳过 `_route` 直接执行 |
| `/full` | **新增** | 强制走 Full 模式：设 `forced_lane="full"` 透传 `handle_user_input`，仍调 `_route` 拆域任务但覆盖 `lane="full"`（Full 需任务拆分，不能跳过路由） |
| `/help`、`/exit` | 全模式生效 | 行为不变 |

> **P0: SLASH_PRE_INTERCEPT** — 斜杠命令**全部**由 REPL 层 `_on_user_input`（repl.py:159）派发给 `chat/commands.py` 处理器，**不进 `handle_user_input`**、不消耗路由 token（B1 修正：原 `handle_user_input` 的 `text.startswith('/')` 死分支已移除——`/clear` 该 reset 会话而非当 prompt 发给 LLM）：
> 1. **车道指定命令**（`/fast`、`/full`）：commands.py 处理器设 `forced_lane` 后经 `ChatCLI._handle_message` 透传调 `handle_user_input(text, forced_lane=...)`。`/fast`→`forced_lane="fast"` 跳过 `_route` 直接执行；`/full`→`forced_lane="full"` 仍调 `_route` 拆域任务但覆盖 `lane="full"`（B2：Full 需任务拆分，不能跳过路由）。
> 2. **会话命令**（`/clear`、`/context`、`/undo`、`/help` 等）：commands.py 处理器直接处理（`cmd_clear`→`reset_session`、`cmd_context`→`context_provider` 等），不进 `handle_user_input`。
> 原因：斜杠命令语义与领域路由无关，REPL 层统一拦截避免无效 LLM token 消耗。
...
#### 5.1.6 `.gitignore` 与 worktree 路径

worktree 落地路径 `.harness/worktrees/` 必须加入 `.gitignore`，否则 worktree 目录（含内部 `.git` 文件）会被主仓库跟踪，引发递归与污染。Step 4 实现前先在 `.gitignore` 追加：
...
> **L0Router 服务范围**：Phase 2 L0Router 只服务 `harness chat` REPL 路径；`harness run` 单次 CLI 模式仍走 `orchestrator.py` → `base_agent.run_agent()`，不经路由判定（单次任务无对话上下文，路由价值有限，且保持 Phase 1 稳定路径不动以降风险）。
...
| 验证项 | 验证目的 | 输出标准 |
|-------|---------|---------|
| **命令行参数名核实** | 确认 flag 真实名称 | `claude --help` 已核对：`-p`/`--print`、`--allowedTools`、`--output-format`、`--tools`、`--permission-mode`、`--model`、`--json-schema`、`--max-budget-usd` 均存在；**CLI 无 `--cwd`**，cwd 由 subprocess 参数控制；`--max-turns`/`--task-budget`/`--session-mirror` 等 SDK 传但 help 未列的 flag 需实测是否接受 |
...
>
> **前置必做（回归安全网）**：当前 `ChatSession.send()` / `cancel()` 无测试覆盖（仅 `test_session_restore.py` 覆盖构造与 session_store）。570 行重构无安全网风险过高。抽底座前先补 `ChatSession.send()` 关键路径快照测试：文本流、工具调用流、取消、重试、rate limit。测试先针对旧 `ChatSession` 写并跑通，重构后底座 + 瘦身 `ChatSession` 须继续通过，作为回归基准。
>
> **⚠️ Spike 修正(架构师.md §4.3/§13)**：`send()` 内部须改造为 **AsyncIterable prompt 流式模式**（非 string prompt 直传）。`can_use_tool` 仅在流式路径注册，string prompt 走非流式路径会 `ValueError`（SDK client.py:100-106，§13 Run E 实测）。改造点：`send()` 内部把 string prompt 包成 `async def _prompt_stream(): yield {"type":"user","session_id":"","message":{"role":"user","content":prompt},"parent_tool_use_id":None}`，再 `client.query(_prompt_stream())` + `receive_response()`。回归测试须覆盖：流式 send 文本流/工具调用流/cancel，**且** L0Router 注入 `can_use_tool` 后回调确实触发（§13 Run G 验证模式）。`permission_mode` 提为 `__init__` 参数（ChatSession 默认 `acceptEdits`，L0Router 默认 `default`），不再硬编码。
...
class BaseAgentSession:
    ...
    - can_use_tool 权限回调挂载（要求 send() 走 AsyncIterable 流式 prompt）
    - 上下文用量查询 (get_context_usage)
    - 会话状态属性 project_dir / model / context_provider / stats（供 cli.session.* 穿透，commands.py 零改动）
    """
    ...
    def __init__(
        ...
        project_dir: str,
        model: str | None = None,
        system_prompt: SystemPromptType = "",   # str | dict；L0Router 直传 EXECUTOR_PROMPT，ChatSession 覆盖钩子动态构建
        allowed_tools: list[str] | None = None,
        permission_mode: str = "acceptEdits",   # ChatSession 默认;L0Router 注入 "default"
        session_store: SessionStore | None = None,
        context_provider: ContextProvider | None = None,  # 从 ChatSession 上提；底座只喂热重启 + system_prompt 构建
        can_use_tool: Callable | None = None,   # L0Router 注入 _lane_guard;ChatSession 传 None
        ...
        self.stats = SessionStats()             # send() 累加；新增 total_cost_usd 字段（/stats Fast 栏 cost 取此）

    def _resolve_system_prompt(self) -> SystemPromptType:
        """system_prompt 解析钩子。默认直返构造值（L0Router 路径：EXECUTOR_PROMPT）。
        ChatSession 覆盖此钩子调 context_provider.build_system_prompt()（返回 preset dict）。"""
        return self.system_prompt

    def _build_options(self, for_rebuild: bool = False) -> ClaudeAgentOptions:
        """构造 ClaudeAgentOptions。挂载 _resolve_system_prompt() 钩子，一次覆盖
        start / _hot_restart_client / _recover_client_after_cancel 三路径（session.py:354/390/422）。"""
        opts = ClaudeAgentOptions(system_prompt=self._resolve_system_prompt(), cwd=self.project_dir, ...)
        return opts

    async def start(self) -> None: ...
    async def close(self) -> None: ...
    async def send(self, prompt: str) -> AsyncIterator[ChatEvent]:
        """内部把 prompt 包成 AsyncIterable 走流式 query(),使 can_use_tool 生效。
        累加 self.stats（turn_count / tool_calls / tokens / total_cost_usd）。"""
...
> **`system_prompt` 类型 = `SystemPromptType`(str | dict)**：`DefaultContextProvider.build_system_prompt()` 返回 `{"type":"preset","preset":"claude_code"}`（dict，`provider.py:121-123`），底座只收 str 会断 ChatSession 的 preset 路径。故 `__init__` 收 `system_prompt: SystemPromptType`，`_resolve_system_prompt()` 钩子默认直返该值（L0Router 直传 `EXECUTOR_PROMPT` 不覆盖），ChatSession 覆盖钩子调 `context_provider.build_system_prompt()`。
>
> **挂载点选 `_build_options()` 非 `send()`**：`_build_options` 在 `start`/`_hot_restart_client`/`_recover_client_after_cancel` 三处都被调（`session.py:354/390/422`），钩子一次覆盖三路径；改 `send()` 会漏 recover/hot-restart。
>
> **会话状态属性上提**：`project_dir`/`model`/`context_provider`/`stats` 从 ChatSession 上提到底座，供 `cli.session.*` 穿透（commands.py 6 处只读访问零改动）。`context_provider` 在底座只喂热重启（`has_context_changed`/`get_history_summary`），默认 provider 下 no-op，符合 Phase 2 推迟压缩。`stats` 新增 `total_cost_usd` 字段，在 `send()` 从 `ResultMessage.total_cost_usd` 累加。
```
...
EXECUTOR_PROMPT = """你是 Harness Agent 的执行体（L0 Executor）。
你直接处理用户的开发需求，有完整的文件读写与命令执行能力。
...
class L0Router:
    ...
    def __init__(self, project_dir: str, model: str | None = None):
        ...
        # Spike 修正(§13): 业务写工具不进 allowed_tools + permission_mode="default"
        # → 强制走 ask 态触发 can_use_tool 按 lane 放行/拒绝
        self._session = BaseAgentSession(
            project_dir=project_dir,
            model=model,
            system_prompt=EXECUTOR_PROMPT,
            allowed_tools=["Read", "Glob", "Grep", "Bash"],  # 业务写不在内
            permission_mode="default",                         # 非 acceptEdits
    ...
    async def _route(self, text: str) -> dict:
        ...
        opts = ClaudeAgentOptions(
            ...
            tools=[],  # 路由判定纯 LLM 判定，彻底禁用内置工具（SDK 映射 --tools ""）
    ...
    async def handle_user_input(
        ...
        # B1: 斜杠命令不进本方法 —— REPL 层 _on_user_input 已派发给 commands.py 处理器。
        #     原 text.startswith('/') 死分支已移除（/clear 该 reset 会话而非发给 LLM）。
        # B2: /fast、/full 由 commands.py 设 forced_lane 后经 _handle_message 透传：
        #     None→自动路由; "fast"→跳过 _route 直接执行; "full"→仍调 _route 拆任务但覆盖 lane

        # 1. 入口设定 lane：forced fast 直接定；自动/forced full 先清 None（防异常残留）
        if forced_lane == "fast":
            self._current_lane = "fast"
        else:
            self._current_lane = None

        # 2. 路由判定 — forced fast 跳过；自动/forced full 调轻量 query()，不进常驻会话
        if forced_lane == "fast":
            ...
            try:
                ...
                yield _build_message("⚠️ 路由判定异常，已降级单领域执行。")
                self._current_lane = "fast"
...
> **⚠️ Spike 修正(架构师.md §4.2/§13)**：常驻会话配置须用 `permission_mode="default"`（**非** `acceptEdits`）+ 业务写工具（`Write/Edit/NoteBookEdit`）**不进** `allowed_tools`。否则工具被预批准/acceptEdits 覆盖，跳过 ask 态，`can_use_tool` 永不触发（§13 Run A/B 实测）。`allowed_tools` 只放 `Read/Glob/Grep/Bash`（Bash 放行给 git 运维）。`_lane_guard` 按 `_current_lane` 放行/拒绝：fast→Allow（含业务写）、full/None→Deny 业务写。验证标准同 §13 Run G：持久会话上 fast→Allow 写入成功、full→Deny 写入被拒。

#### Step 4: Full 模式 — CLI 子进程 + worktree 隔离 + merge 收口（~1 天）

增加 `_extract_cost()` 解析费用；`_spawn_cli()` 通过 `create_subprocess_exec(cwd=worktree)` 设置子进程工作目录（CLI 无 `--cwd` flag）；`--verbose` **必须带**（Step0 实测：`-p --output-format stream-json` 不带 `--verbose` 则 exit=1；带后 stderr 空、不污染 stdout）；`_extract_result_text()` 取 `type:"result"` 行的 `result` 字段（非 `content[0].text`）；L2 领域子进程用 `--tools` 白名单（可见性）+ `--permission-mode acceptEdits`（worktree 隔离下安全，`-p` 不弹窗）。**⚠️ Windows CLI 路径(§13 Run F)**：`_spawn_cli` 不传 `"claude"` 字面量，用 `_resolve_cli_path()`（`shutil.which("claude")` 解析全路径，Win→`claude.CMD` shim），否则 `CreateProcess` 不解析 PATHEXT → `FileNotFoundError`。
...
ChatCLI 入口增加模式开关（默认走 L0）。新增 `/fast`/`/full` 命令（由 `chat/commands.py` 拦截，设 `forced_lane` 经 `ChatCLI._handle_message` 透传 `handle_user_input`：`/fast` 跳过 `_route`、`/full` 仍路由拆任务但覆盖 lane）。`/undo` 仅 Fast Lane 生效。`/clear` 经 `router.rebuild()` 重置常驻会话与 `_current_lane`。
...
class ChatCLI:
    def __init__(self, project_dir: str, model: str | None = None,
        ...
        self.model = model
        self.router = L0Router(project_dir, model, context_provider=context_provider)  # Phase 2 核心
        self.renderer = ChatRenderer(...)

    @property
    def session(self) -> BaseAgentSession:
        """穿透到 router 常驻底座 —— commands.py 的 cli.session.* 不变（5 命令零改动）。"""
    ...
    async def switch_project(self, new_dir: str) -> None:
        if self.router._full_active:
            self.renderer.render_command_result("Full 进行中，无法切换"); return
        self.project_dir = str(Path(new_dir).resolve())
        await self.router.rebuild(project_dir=self.project_dir)

    async def switch_model(self, model: str) -> None:
        if self.router._full_active:
            self.renderer.render_command_result("Full 进行中，无法切换"); return
        self.model = model
        await self.router.rebuild(model=model)

    async def reset_session(self) -> None:
        if self.router._full_active:
            self.renderer.render_command_result("Full 进行中，无法切换"); return
        await self.router.rebuild()           # 用当前 project_dir/model 重建

    def request_exit(self) -> None: ...
```

> **commands.py 接口契约保持**：`/context`/`/project`(显示)/`/model`(显示)/`/clear`/`/exit` 经 `cli.session` 代理 + `cli.switch_*`/`reset_session`/`request_exit` 走 `router.rebuild`，**5 命令零改动**。
>
> **`/stats` 改一处（两栏）—— 唯一破口**：`cmd_stats` 分两段显示：
> 1. **常驻会话（Fast Lane）**：`cli.session.stats`（turns / tools / input_tokens / output_tokens / total_cost_usd）
> 2. **Full 累计**：`cli.router._full_cost` + `cli.router._full_tokens`（input/output）
>
> 理由：Full 是贵的路径，cost 藏出 `/stats` 会误导；塞进同一个 `SessionStats` 又混语义（turns 对不上）。两栏最诚实。零改动退路（`/stats` 只显 Fast，Full 累计只在收口 summary 报）会让用户查总花费扑空，不推荐。
>
> **`switch_*` 与 Full 进行中拒绝**：`_run_full` 进出置 `router._full_active`，`switch_project`/`switch_model`/`reset_session` 检测到则 `render_command_result("Full 进行中，无法切换")` 直接返回。理由：rebuild 改 `project_dir`/`model` 会抽走 `_run_full` 的 worktree 路径与 `merge_worktree(self.project_dir,...)` 落点，merge 落错地方；放行需在 `_run_full` 入口快照 project_dir 全程用快照，且"切项目中途跑 Full"的 UX 本身就乱，不值。
...
| 7 | **动态权限硬约束** | 常驻会话用 `permission_mode="default"` + 业务写工具不进 `allowed_tools`（强制走 ask 态）；`can_use_tool` 回调按 lane 放行/拒绝：Full 模式下 `Write/Edit/NotebookEdit` 被拒、`Bash` 放行（运维），Fast Lane 下全放行，`None`（异常残留）按 Full 最严处理。`send()` 走 AsyncIterable 流式 prompt 触发回调。验证依据：§13 Spike Run G（持久会话 fast→Allow 写入成功、full→Deny 写入被拒） |
| 8 | **CLI 子进程契约** | `claude -p` headless 调用成功执行并解析输出；与 Step 0 Spike 输出的契约表对齐；Windows 上经 `shutil.which` 解析 CLI 路径（§13 Run F） |
...
| 16 | **斜杠命令兼容** | `/undo` 仅 Fast Lane 生效；`/context`/`/project`(显示)/`/model`(显示)/`/clear`/`/exit` 经 session 代理零改动；`/stats` 改两栏（Fast+Full）；`/fast`/`/full` 新增命令强制指定车道；`switch_*`/`reset` 在 `_full_active` 时拒绝 |
...
| 20 | **斜杠命令前置拦截（P0: SLASH_PRE_INTERCEPT）** | REPL 层 `_on_user_input` 将 `/` 输入派发给 commands.py，不进 `handle_user_input`、不消耗路由 token；`handle_user_input` 无 `/` 分支（B1）。`/fast`/`/full` 经 `forced_lane` 形参强制车道：fast 跳过路由、full 仍路由拆任务但覆盖 lane（B2） |
...
| Step 5 | REPL 集成 L0Router + 斜杠命令适配（`/fast`/`/full`） | P0 | ~0.5d |
...
| 14 | **P0: 斜杠命令未前置拦截 → 无效消耗路由 LLM token** | `/clear`、`/context` 等命令支付不必要的模型调用成本 | REPL 层 `_on_user_input` 派发 `/` 输入到 commands.py 处理器，不进 `handle_user_input`（B1）；`/fast`/`/full` 经 `forced_lane` 透传（B2） |
...
| ADR-6 | 权限用 **`can_use_tool` 动态准入**，不用静态 `allowed_tools` 放行业写 | L0 Fast Lane 需要写权限（自执行），Full 模式 L0 只应派发。静态白名单无法同时满足两个场景，动态回调按 lane 来。**⚠️ Spike 修正(§13)**：`can_use_tool` 仅在 ask 态生效，故 `allowed_tools` 仍用于放行只读工具（`Read/Glob/Grep/Bash`），**业务写工具不进** `allowed_tools` + `permission_mode="default"`（非 `acceptEdits`）才能强制走 ask 态触发回调；且 `can_use_tool` 要求 AsyncIterable 流式 prompt |
